// @ts-nocheck — legacy seed file, not used in production
import mongoose from "mongoose";
import dotenv from "dotenv";
import User from "./models/User";
// Legacy models removed — seed.ts is a legacy file and should be replaced or updated
...
const seedDatabase = async () => {
  try {
    ...
    const courses = [
      {
        ...
        description:
          "Explore the fundamental units of life - cells. Learn about cell structure, function, and processes.",
      ...
      },
      {
        ...
        description:
          "Deep dive into DNA, genes, heredity, and genetic engineering.",
      ...
      },
      {
        ...
        description:
          "Understanding ecosystems, biodiversity, and environmental challenges.",
      ...
      },
      {
        ...
        description:
          "Study molecular mechanisms in biological activity including DNA replication and protein synthesis.",
        teacherId: createdTeachers[0]._id,
        price: 449,
        thumbnail:
    ...
    for (const course of createdCourses) {
      const courseSections = createdSections.filter(
        (s: any) => s.courseId.toString() === course._id.toString(),
      );
      ...
      for (let i = 0; i < numQuizzes; i++) {
        const lesson =
          courseLessons[i * Math.floor(courseLessons.length / numQuizzes)];
        quizzesData.push({
          lessonId: lesson._id,
    ...
    for (const enrollment of createdEnrollments) {
      const course = createdCourses.find(
        (c: any) => c._id.toString() === enrollment.courseId.toString(),
      );
      ...
      if (course) {
        ...
        const numResults = Math.max(
          1,
          Math.floor(courseQuizzes.length * (0.5 + Math.random() * 0.5)),
        );
        const shuffledQuizzes = [...courseQuizzes].sort(
          () => Math.random() - 0.5,
        );
...
function getSectionTitle(courseIndex: number, sectionIndex: number): string {
  ...
  return (
    titles[courseIndex]?.[sectionIndex] || `Advanced Topics ${sectionIndex + 1}`
  );
}
...
function getAnswerText(questionIndex: number, optionIndex: number): string {
  ...
  return (
    answers[questionIndex % answers.length]?.[optionIndex] ||
