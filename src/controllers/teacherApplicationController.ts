...
export const getTeacherApplications = async (_req: Request, res: Response) => {
  try {
    const applications = await TeacherApplication.find({}).sort({
      createdAt: -1,
    });
...
export const reviewTeacherApplication = async (req: Request, res: Response) => {
  try {
    ...
    if (action === "evaluate") {
      ...
      if (zoomLink) {
        const template = teacherEvaluationScheduledTemplate(
          application.name,
          zoomLink,
        );
        try {
    ...
    if (action === "accept") {
      ...
      const appStageIds =
        Array.isArray(application.stageIds) && application.stageIds.length > 0
          ? application.stageIds
          : application.stageId
            ? [application.stageId]
            : [];
      const appSubjectIds = Array.isArray(application.subjectIds)
        ? application.subjectIds
        : [];
      const appGradeIds = Array.isArray((application as any).gradeIds)
        ? (application as any).gradeIds
        : [];
      ...
      if (!resolvedGradeIds.length && appStageIds.length > 0) {
        const grades = await Grade.find({ stageId: { $in: appStageIds } })
          .select("_id")
          .lean();
      ...
      const template = teacherApplicationAcceptedTemplate(
        teacher.name,
        teacher.email,
        tempPassword,
        teacher.subject,
    ...
    if (action === "reject") {
      ...
      const updated = await application.save();
      const template = teacherApplicationRejectedTemplate(
        application.name,
        rejectionReason,
      );
      try {
    ...
    res
      .status(400)
      .json({
        message: 'Invalid action. Use "evaluate", "accept", or "reject".',
      });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
